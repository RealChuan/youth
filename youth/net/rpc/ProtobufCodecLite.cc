...
void ProtobufCodecLite::fillEmptyBuffer(
    net::Buffer *buf, const google::protobuf::Message &message) {
  ...
  (void)byte_size;
  int32_t len = net::SocketFunc::hostToNetwork32(
      static_cast<int32_t>(buf->readableBytes()));
  buf->prepend(&len, sizeof len);
}

void ProtobufCodecLite::onMessage(const net::TcpConnectionPtr &conn,
                                  net::Buffer *buf, Timestamp receiveTime) {
  while (buf->readableBytes() >=
    ...
    if (len > kMaxMessageLen || len < kMinMessageLen) {
      ...
      if (m_rawCb &&
          !m_rawCb(conn, std::string_view(buf->peek(), kHeaderLen + len),
                   receiveTime)) {
...
int ProtobufCodecLite::serializeToBuffer(
    const google::protobuf::Message &message, net::Buffer *buf) {
