...
public:
  ...
  const static int kMaxMessageLen =
      64 * 1024 * 1024; // same as m_codecstream.h kDefaultTotalBytesLimit
  ...
  typedef std::function<bool(const net::TcpConnectionPtr &, std::string_view,
                             core::Timestamp)>
      RawMessageCallback;

  typedef std::function<void(const net::TcpConnectionPtr &, const MessagePtr &,
                             Timestamp)>
      ProtobufMessageCallback;

  typedef std::function<void(const net::TcpConnectionPtr &, net::Buffer *,
                             Timestamp, ErrorCode)>
      ErrorCallback;
  ...
  ProtobufCodecLite(const ::google::protobuf::Message *prototype,
      ...
      : m_prototype(prototype), m_tag(tagArg), m_messageCallback(messageCb),
        m_rawCb(rawCb), m_errorCallback(errorCb),
  ...
  ErrorCode parse(const char *buf, int len,
                  ::google::protobuf::Message *message);
  void fillEmptyBuffer(net::Buffer *buf,
                       const google::protobuf::Message &message);
  ...
  static void defaultErrorCallback(const net::TcpConnectionPtr &, net::Buffer *,
                                   Timestamp, ErrorCode);
...
public:
  ...
  typedef std::function<void(const net::TcpConnectionPtr &,
  ...
  explicit ProtobufCodecLiteT(
      ...
      : m_messageCallback(messageCb),
        m_codec(&MSG::default_instance(), TAG,
  ...
  void onRpcMessage(const net::TcpConnectionPtr &conn,
                    const MessagePtr &message, Timestamp receiveTime) {
    m_messageCallback(conn, std::static_pointer_cast<MSG>(message),
                      receiveTime);
  }
