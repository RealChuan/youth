...
{
    LOG_DEBUG << conn->localAddress().ipAndPort() << " -> " << conn->peerAddress().ipAndPort()
              << " is " << (conn->connected() ? "UP" : "DOWN");
    // do not call conn->forceClose(), because some users want to register message
    // callback only.
}

void defaultMessageCallback(const TcpConnectionPtr &, Buffer *buf, DateTime)
{
    buf->retrieveAll();
}
...
{
    LOG_DEBUG << "TcpConnection::dtor[" << m_name << "] at " << this << " fd=" << m_channelPtr->fd()
              << " state=" << stateToString();
...
void TcpConnection::handleRead(DateTime receiveTime)
{
    m_eventLoop->assertInLoopThread();
    int savedErrno = 0;
