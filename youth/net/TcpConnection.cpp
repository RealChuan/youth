...
TcpConnection::TcpConnection(EventLoop *loop, const std::string &name,
                             int sockfd, const TcpAddressInfo &localAddr,
                             const TcpAddressInfo &peerAddr)
    : m_eventLoop(CHECK_NOTNULL(loop)), m_name(name), m_state(kConnecting),
      m_reading(true), m_socketPtr(new Socket(sockfd)),
      m_channelPtr(new Channel(loop, sockfd)), m_localAddr(localAddr),
...
TcpConnection::~TcpConnection() {
  LOG_DEBUG << "TcpConnection::dtor[" << m_name << "] at " << this
            << " fd=" << m_channelPtr->fd() << " state=" << stateToString();
...
void TcpConnection::send(Buffer *buf) {
  if (m_state == kConnected) {
    if (m_eventLoop->isInLoopThread()) {
      ...
      void (TcpConnection::*fp)(const std::string_view &message) =
          &TcpConnection::sendInLoop;
      m_eventLoop->runInLoop(std::bind(fp,
                                       this, // FIXME
...
void TcpConnection::setTcpNoDelay(bool on) { m_socketPtr->setTcpNoDelay(on); }
...
void TcpConnection::handleWrite() {
  ...
  if (m_channelPtr->isWriting()) {
    ssize_t n = SocketFunc::write(m_channelPtr->fd(), m_outputBuffer.peek(),
...
void TcpConnection::sendInLoop(const void *data, size_t len) {
  ...
  if (!faultError && remaining > 0) {
    ...
    if (oldLen + remaining >= m_highWaterMark && oldLen < m_highWaterMark &&
      ...
      m_eventLoop->queueInLoop(std::bind(
