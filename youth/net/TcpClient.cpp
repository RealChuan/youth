...
TcpClient::TcpClient(EventLoop *loop, const TcpAddressInfo &serverAddr,
                     const std::string &name)
    : m_eventLoop(CHECK_NOTNULL(loop)),
      m_connectorPtr(new Connector(loop, serverAddr)), m_name(name),
      m_connectionCallback(defaultConnectionCallback),
      m_messageCallback(defaultMessageCallback), m_retry(false),
      m_connect(true), m_nextConnId(1) {
  m_connectorPtr->setNewConnectionCallback(
      std::bind(&TcpClient::newConnection, this, std::placeholders::_1));
  // FIXME setConnectFailedCallback
  LOG_INFO << "TcpClient::TcpClient[" << m_name << "] - connector "
           << m_connectorPtr.get();
}
...
TcpClient::~TcpClient() {
  ...
  if (conn) {
    ...
    CloseCallback cb =
        std::bind(&net::removeConnection, m_eventLoop, std::placeholders::_1);
...
void TcpClient::newConnection(int sockfd) {
  ...
  char buf[32];
  snprintf(buf, sizeof buf, ":%s#%d", peerAddr.ipAndPort().c_str(),
           m_nextConnId);
  ++m_nextConnId;
  ...
  TcpConnectionPtr conn(
      new TcpConnection(m_eventLoop, connName, sockfd, localAddr, peerAddr));
