...
LogAsync::LogAsync(int flushInterval)
    : m_timestamp(Timestamp::currentTimestamp()), m_refreshTime(flushInterval),
      m_running(false), m_thread(std::bind(&LogAsync::threadFunc, this)),
      m_mutex(), m_cond(), m_currentBuffer(new Buffer),
  ...
  while (m_running) {
    ...
    if (buffersToWrite.size() > 25) {
      ...
      snprintf(buf, sizeof buf,
               ...
               m_timestamp.microSecondsToString().c_str(),
